...
\begin{itemize}
    ...
    \item multi_apply\_linear: linear operator which provides the product of the systems
    Jacobian with a given set of vectors
    \item multi_solve\_linear: solve the linear system defined by the jacobian on the
...
given workflow. \method{build\_sequence} should be called recursively on all child groups inside a
containing group.
...
There are two different types of \classname{NameSpace}: automatic connection and manual connection.
\subsection{Auto Connect (Global Variables)}
\subsection{Manual Connection (Local Variables)}
\subsection{Mass connection of grouped variables}
