...
class SetTotalTP(Assembly):
    ...
    def __init_(self, themo_data=props.janaf, dtype=np.float, test_resid=False):
        ...
        if not test_resid:
            #don't put a solver in if you want to test residual derivatives
            chem_eq.solve_nonlinear = CEAGaussSeidel()
            chem_eq.solve_linear = ScipyGMRES()

        chem_eq.add('n2ls', N2LS_TP(thermo, dtype),
                    promote=("n", 'init_prod_amounts', 'b', 'mu', 'rhs', 'ch'))
        chem_eq.add('ls2pi', LinearSystem(self.num_element+1,
                    A_name="ch", x_name="pi", b_name="rhs"), promote="*")
        ...
        self.add('TP2ls', TotalRHS(thermo, dtype),
                promote=('n', 'b', 'ch', 'rhs_T', 'rhs_P', 'lhs_TP'))
        ...
        self.add('ls2p', LinearSystem(self.num_element+1,
                 ...
                 promote="*")
        self.add('tp2tot', TotalCalcs(thermo),
                promote=("n",'result_T','result_P','gamma'))
        ...
        #Boundary Outputs
        self.add('ht_english',
                 UnitComp('ht_SI', 'cal/g', 'ht', 'Btu/lbm'), promote="*")
        self.add('S_english',
                UnitComp('S_SI', 'cal/(g*degK)', 'S', 'Btu/(lbm*degR)'), promote="*")
        self.add('Cp_english',
                UnitComp('Cp_SI', 'cal/(g*degK)', 'Cp', 'Btu/(lbm*degR)'), promote="*")
        self.add('Cv_english',
                UnitComp('Cv_SI', 'cal/(g*degK)', 'Cv', 'Btu/(lbm*degR)'), promote="*")
        self.add('rhot_english',
                UnitComp('rhot_SI', "g/cm**3", 'rhot', "lbm/ft**3"), promote="*")
